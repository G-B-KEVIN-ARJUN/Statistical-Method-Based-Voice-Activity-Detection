...
def stft(x, n_fft=512, win_length=400, hop_length=160, window='hamming'):
    if window == 'hanning': # Windowing function: 'hanning' 
        window = np.hanning(win_length)
    elif window == 'hamming': # Windowing function 'hamming'
        window = np.hamming(win_length)
    elif window == 'rectangle':# Windowing function  'rectangle'
...
def mhvals(*args):
    nargin = len(args) #constants and calculations explained in the code
...
# Plotting the original signal and VAD result
plt.subplot(2, 1, 1)
plt.plot(signal)
...
plt.subplot(2, 1, 2)
plt.plot(vad)
plt.title("Voice Activity Detecting Signal")
plt.xlabel("time")
plt.ylabel("amplitude")
